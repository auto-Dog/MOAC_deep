...
def per_pkt_transmission(args, MM, TransmittedSymbols):
    ...
    # if args.Estimator == 5:
        ...
        # print(samples)  # debug
        # print(MM,hh.shape)    # debug
        output = inference(samples.flatten(),'../model_new.pth',MM,LL,hh,args.EsN0dB)
        # return output
        outputs[5] = output
...
def test():
    ...
    args.Estimator = 5
    output,_ = per_pkt_transmission(args, MM, TransmittedSymbols.copy())
    MSE5 = np.mean(np.power(np.abs(output - target),2))
    print('MSE5 = ', MSE5)
